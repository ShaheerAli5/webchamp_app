...
class ContactProvider extends ChangeNotifier {
  ...
  List<dynamic> get filteredContacts {
    ...
    final filtered = _contacts.where((contact) {
      ...
      if (contactLabels is List) {
        return contactLabels.any((label) {
          ...
          return title.toLowerCase() == _selectedLabel!.toLowerCase() || uid == _selectedLabel;
        });
      }
      return false;
    }).toList();
    ...
    debugPrint(
      '🏷️ [LABELS] Filtered contacts count: ${filtered.length} for label: $_selectedLabel',
    );
    return filtered;
  }
  ...
  Future<void> loadCachedData() async {
    try {
      ...
      final String? contactsJson = prefs.getString(
        'cached_contacts_${_activeUserId ?? 'anon'}',
      );
      if (contactsJson != null) {
        ...
        if (decoded.isNotEmpty) {
          ...
          _hasMore = true;
          debugPrint(
            '💾 [CACHE] Loaded ${_contacts.length} contacts from persistent storage',
          );
          notifyListeners();
        }
      }
      ...
      final int cachedUnread =
          prefs.getInt('cached_unread_${_activeUserId ?? 'anon'}') ?? 0;
      if (cachedUnread > 0) {
  ...
  Future<bool> getContacts({
    ...
    if (autoLoadAll &&
        _contactsFullyLoaded &&
        !refresh &&
        !isSearching &&
        !loadMore) {
      debugPrint(
        'ℹ️ [CONTACTS] Already fully loaded. Switching to background page 1 sync.',
      );
      autoLoadAll = false;
    }
    ...
    if (!loadMore && !isRecursiveCall) {
      ...
      _activeRequestSessionId = DateTime.now().millisecondsSinceEpoch
          .toString();
      debugPrint(
        '🆔 [CONTACTS] NEW SESSION: $_activeRequestSessionId (Search: $search)',
      );
    }
    ...
    try {
      debugPrint(
        '🚀 [CONTACTS] FETCH START - Page: $_currentPage, Search: $search',
      );
      ...
      if (autoLoadAll && !loadMore && !isSearching) {
        return await _fetchAllContactsParallel(
          search,
          effectivePerPage,
          refresh,
          currentSessionId,
      ...
      if (currentSessionId != _activeRequestSessionId) {
        debugPrint(
          '🛑 [CONTACTS] Session mismatch. Discarding Page $_currentPage.',
        );
        return false;
      }
      ...
      for (var contact in newContacts) {
        if (contact is Map) {
          contact['first_name'] = _sanitizeText(
            contact['first_name']?.toString(),
          );
          contact['last_name'] = _sanitizeText(
            contact['last_name']?.toString(),
          );
          contact['full_name'] = _sanitizeText(
      ...
      if (newContacts.isEmpty) {
        ...
        final List<MapEntry<dynamic, DateTime>> timedContacts = _contacts
            .map(
              (c) => MapEntry(
                c,
                Helpers.toUtc(c['latest_message'] ?? c['updated_at']),
              ),
            )
            .toList();
        ...
        if (autoLoadAll && _hasMore) {
          debugPrint(
            '⏳ [CONTACTS] Auto-loading next page ($_currentPage + 1)...',
          );
          ...
          if (addedInThisPage == 0 && loadMore) {
            debugPrint(
              '🛑 [CONTACTS] No new unique contacts added in this page, stopping recursion.',
            );
            _hasMore = false;
          }
        }
  ...
  Future<bool> _fetchAllContactsParallel(
    String? search,
    int perPage,
    bool refresh,
    String sessionId,
    ...
    try {
      while (!endReached && sessionId == _activeRequestSessionId) {
        debugPrint(
          '🚀 [BATCH] Fetching pages $_currentPage to ${_currentPage + concurrency - 1}',
        );
        ...
        if (successfulInBatch == 0) {
          debugPrint(
            '🛑 [BATCH] Entire batch failed. Stopping parallel fetch.',
          );
          endReached = true;
        }
        ...
        if (buffer.length >= 120 ||
            endReached ||
            pagesFetchedInCurrentBatch >= uiUpdateBatch) {
          debugPrint(
            ' [UI] Batch Update: Adding ${buffer.length} contacts (Total: ${_contacts.length + buffer.length})',
          );
  ...
  void _updatePaginationState(
    ...
    dynamic paginateInfo =
        clientModels?['contactsPaginatePage'] ??
        result['pagination'] ??
        result['data']?['pagination'];

    if (paginateInfo is Map) {
      _total =
          _toInt(paginateInfo['total']) ??
    ...
    if (result is Map) {
      ...
      if (data is Map) {
        ...
        if (data['contacts'] is Map) {
          return (data['contacts'] as Map).values.toList();
        }
      ...
      if (result['contacts'] is List) {
        return result['contacts'];
      }
      if (result['contacts'] is Map) {
        return (result['contacts'] as Map).values.toList();
      }
    }
    return [];
  }
  ...
  Future<bool> loadContactByUid(String uid) async {
    ...
    try {
      ...
      if (result != null && result is Map) {
        ...
        final contactData =
            (clientModels is Map ? clientModels['contact'] : null) ??
        ...
        if (contactData != null && contactData is Map) {
          ...
          debugPrint(
            '⚠️ [CONTACT] Fetched chat box data but no contact object found for $uid',
          );
        }
      }
  ...
  Future<bool> createContact({
    ...
    try {
      ...
      if (!isSuccessful) {
        throw Exception(result['message'] ?? 'Failed to add contact');
      }
        ...
        final int newCount =
            _toInt(
              result['unread_count'] ??
                  ...
                  result['client_models']?['unreadMessagesCount'],
            ) ??
            0;
  ...
  Future<void> markContactAsRead(String contactUid, {String? messageId}) async {
    ...
    for (int i = 0; i < _contacts.length; i++) {
      ...
      if (contact is Map && (contact['_uid'] ?? contact['uid']) == contactUid) {
        previousCount =
            Helpers.toInt(
              contact['unread_messages_count'] ?? contact['unread_count'],
            ) ??
            0;
        contactIndex = i;
        break;
    ...
    if (messageId == null && previousCount <= 0) {
      debugPrint(
        'ℹ️ [MARK READ] Contact $contactUid already has 0 unread. Skipping.',
      );
      return;
    }
    ...
    debugPrint(
      '📩 [MARK READ] START - Contact: $contactUid, MessageID: $messageId, Prev Contact Unread: $previousCount, Prev Global: $previousGlobalCount',
    );
    ...
    if (contactIndex != -1) {
      ...
      if (messageId == null) {
        ...
        _globalUnreadCount = (_globalUnreadCount - previousCount).clamp(
          0,
          999999,
        );
      } else {
        int currentUnread =
            Helpers.toInt(
              updatedContact['unread_messages_count'] ??
        ...
        if (msg is Map) {
          ...
          if (messageId != null) {
            if (id == messageId && msg['status'] != 'read') {
              debugPrint(
                '📝 [MARK READ] Updating Message $id: ${msg['status']} -> read',
              );
            ...
            if (msg['status'] != 'read') {
              debugPrint(
                '📝 [MARK READ] Updating Incoming Message $id: ${msg['status']} -> read',
              );
              final updatedMsg = Map<String, dynamic>.from(msg);
              updatedMsg['status'] = 'read';
              _messages[i] = updatedMsg;
    ...
    debugPrint(
      '🔔 [MARK READ] Local state updated. New Global: $_globalUnreadCount',
    );
    notifyListeners();
    _saveToPersistentCache();
    ...
    try {
      debugPrint(
        '📡 [MARK READ] API Request: contact_uid=$contactUid, message_id=$messageId',
      );
      ...
      if (result is Map &&
          (result['unread_count'] != null ||
              result['data']?['unread_count'] != null)) {
        final serverCount = Helpers.toInt(
          result['unread_count'] ?? result['data']?['unread_count'],
        );
        if (serverCount != null) {
  ...
  Future<bool> getContactChatBoxData(
    String contactUid, {
    ...
    debugPrint(
      '🚀 [CHAT] getContactChatBoxData START: $contactUid (Force: $force, Refresh: $refresh, PollOnly: $pollOnly)',
    );
    if (!showLoading &&
        !refresh &&
        !force &&
      ...
      debugPrint(
        '⏳ [CHAT] Ignoring background polling for inactive chat: $contactUid (Active: $_activeChatUid)',
      );
      return false;
    }
    ...
    if (_activeChatUid != contactUid) {
      debugPrint(
        '🔀 [CHAT] Switching from $_activeChatUid to $contactUid. Clearing old messages.',
      );
    ...
    if (_isFetchingChat &&
        _selectedContact?['uid'] == contactUid &&
        !force &&
        !refresh) {
    ...
    try {
      ...
      if (!pollOnly) {
        requests.add(
          _repository.getContactChatBoxData(
            contactUid,
            refresh: refresh,
            cancelToken: _chatCancelToken,
          ),
      ...
      if (contactUid != _activeChatUid) {
        debugPrint(
          '🛑 [CHAT] UID mismatch after fetch. Discarding results for $contactUid (Active: $_activeChatUid)',
        );
      ...
      final dynamic safeClientModels = (clientModels is Map)
          ? clientModels
          : null;
      ...
      final List<dynamic> rawNewMessages = _extractMessagesFromResponse([
        result,
        chatResult,
      ]);
      ...
      if (chatResult is Map) {
        ...
        final paginate =
            clientModels?['whatsappMessageLogsPaginatePage'] ??
      ...
      if (rawNewMessages.isNotEmpty) {
        ...
        if (firstMsg is Map) {
          debugPrint(
            'ℹ️ [CHAT] Latest message status: ${firstMsg['status']}, is_incoming: ${firstMsg['is_incoming_message']}',
          );
        }
      }
      ...
      debugPrint(
        '🔄 [MERGE] Starting merge for contact: $contactUid. Local messages: ${_messages.length}',
      );

      final Set<String> localIdsBefore = _messages
          .map(
            (m) =>
      ...
      if (rawNewMessages.isNotEmpty) {
        debugPrint(
          '📡 [SYNC] Backend messages received: ${rawNewMessages.length}',
        );
        for (var msg in rawNewMessages) {
          ...
          if (idStr != null) {
            ...
            if (msg['is_incoming_message'] == 0 ||
                msg['is_incoming_message'] == '0' ||
                msg['is_incoming_message'] == false) {
              ...
              uniqueMap.removeWhere((key, m) {
                ...
                if (mText == msgText && mText.isNotEmpty && mText != 'Media') {
                  ...
                  if (mTime.difference(msgTime).abs().inSeconds < 60) {
                    removedOptimistic = true;
                    debugPrint(
                      '✂️ [MERGE] Content match found: "$mText". Removing $key',
                    );
                    return true;
                  }
                }
                ...
                if (mType != null && mType == msgType && mType != 'text') {
                  ...
                  if (linkMatch &&
                      mTime.difference(msgTime).abs().inSeconds < 60) {
                    removedOptimistic = true;
                    debugPrint(
                      '✂️ [MERGE] Media match found for file: $mLink. Removing $key',
                    );
                    return true;
                  }
                  ...
                  if (mTime.difference(msgTime).abs().inSeconds < 10) {
                    removedOptimistic = true;
                    debugPrint(
                      '✂️ [MERGE] Time-based media match found. Removing $key',
                    );
                    return true;
                  }
                }
              ...
              if (removedOptimistic) {
                debugPrint(
                  '✂️ [MERGE] Deduplicated optimistic message with backend ID: $idStr',
                );
              } else if (localIdsBefore.contains(idStr)) {
                debugPrint(
                  '⚠️ [MERGE] Server ID $idStr already existed. Overwriting.',
                );
              }
            }
      ...
      for (var entry in uniqueMap.entries) {
        ...
        if (existingMsg != null &&
            ...
            msg['status'] != 'read') {
          debugPrint(
            '🛡️ [MERGE] Preserving local "read" status for message $id',
          );
          if (msg is Map) {
      ...
      for (var m in dedupedList) {
        final id =
            Helpers.getMessageId(m) ??
            'temp_${m.hashCode}_${DateTime.now().microsecondsSinceEpoch}';
        if (!finalUniqueMap.containsKey(id)) {
          finalUniqueMap[id] = m;
        } else {
          debugPrint(
            '🛡️ [MERGE] Final pass caught duplicate ID: $id. Filtering it out.',
          );
        }
      }
      ...
      if ((errStr.contains("Too many requests") ||
              errStr.contains("Too Many Attempts") ||
              errStr.contains("429")) &&
          _chatBoxRetryCount < 3) {
        _chatBoxRetryCount++;
        ...
        debugPrint(
          '🔄 [CHAT] Rate limited (429). Retry $_chatBoxRetryCount/3 in ${backoff}s...',
        );
        await Future.delayed(Duration(seconds: backoff));
        return await getContactChatBoxData(
          contactUid,
  ...
  Future<void> loadMoreChatMessages(String contactUid) async {
    if (_isLoadingMoreChat || !_hasMoreChat) {
      debugPrint(
        '⏳ [CHAT] Skipping load more: loading=$_isLoadingMoreChat, hasMore=$_hasMoreChat',
      );
      return;
    }
    ...
    try {
      ...
      final chatResult = await _repository.getChatHistory(
        contactUid,
        page: nextPage,
      );
      ...
      if (contactUid != _activeChatUid) {
        debugPrint(
          '🛑 [CHAT] UID mismatch in loadMore. Discarding results for $contactUid (Active: $_activeChatUid)',
        );
        return;
      }

      final List<dynamic> newMessages = _extractMessagesFromResponse([
        chatResult,
      ]);
  ...
  void _logChatPagination(
    ...
    if (messages.isNotEmpty) {
      final sorted = List.from(messages)
        ..sort((a, b) => _getDateTime(a).compareTo(_getDateTime(b)));
    ...
    if (rawResponse is Map) {
      final paginate =
          rawResponse['client_models']?['whatsappMessageLogsPaginatePage'] ??
  ...
  bool _isSameMessageList(List<dynamic> list1, List<dynamic> list2) {
    ...
    for (int i = 0; i < list1.length; i++) {
      ...
      if (m1 is Map && m2 is Map) {
        final id1 =
            m1['whatsapp_message_id'] ?? m1['wamid'] ?? m1['_uid'] ?? m1['uid'];
        final id2 =
            m2['whatsapp_message_id'] ?? m2['wamid'] ?? m2['_uid'] ?? m2['uid'];
      ...
      if (result is List) {
        ...
        if (clientModels is Map) {
          ...
          if (logs is Map && logs.isNotEmpty) {
            ...
            currentResultMessages = msgs is Map
                ? msgs.values.toList()
                : (msgs is List ? msgs : []);
          }
        }
  ...
  Future<bool> sendMessage({
    ...
    if (isExpired) {
      ...
      if (replyToMessageId != null) {
        failedMessage['reply_to_message_id'] = replyToMessageId;
      }
    ...
    final optimisticMessage = {
      ...
      if (replyToMessageId != null) {
        optimisticMessage['reply_to_message_id'] = replyToMessageId;
      }
    ...
    try {
      ...
      final index = _messages.indexWhere(
        (m) => m['whatsapp_message_id'] == tempId || m['local_id'] == tempId,
      );
      if (index != -1 && result is Map) {
        ...
        final List<dynamic> sentMessages = _extractMessagesFromResponse([
          result,
        ]);
        final Map<String, dynamic>? serverMsg =
            (sentMessages.isNotEmpty && sentMessages.first is Map)
        ...
        if (serverMsg != null) {
          ...
          if (serverId != null) {
            ...
            if (existingIndex != -1 && existingIndex != index) {
              debugPrint(
                '🚨 [COLLISION] Server ID $serverId ALREADY EXISTS at index $existingIndex. Removing temp message at $index to avoid duplicate keys.',
              );
              ...
              debugPrint(
                '✅ [SEND] Successfully updated temp ID $tempId to server ID $serverId',
              );
            }
          }
        } else {
          debugPrint(
            '⚠️ [SEND] No server message extracted from response, using raw result',
          );
          _messages[index] = {..._messages[index], ...result, 'status': 'sent'};
        }
        ...
        debugPrint(
          '⚠️ [SEND] Could not find temp message $tempId to update after API success (Index: $index)',
        );
      }

      // Background sync to stay in sync with server state
      debugPrint('🔄 [SEND] Triggering background sync');
      getContactChatBoxData(
        contactUid,
        showLoading: false,
        force: true,
        refresh: true,
      ...
      if (displayError.toLowerCase().contains('24 hour') ||
          displayError.toLowerCase().contains('24hour')) {
  ...
  Future<void> deleteMessage({
    ...
    debugPrint(
      '🗑️ [DELETE] Message ID: $messageId, ForEveryone: $forEveryone',
    );

    // 🛡️ Find the message index
    final index = _messages.indexWhere((m) {
      final id =
          (m['whatsapp_message_id'] ?? m['wamid'] ?? m['_uid'] ?? m['uid'])
              .toString();
      return id == messageId;
    });
  ...
  void _updateContactLatestMessage(
    String contactUid,
    ...
    if (contactIndex != -1) {
      ...
      final List<MapEntry<dynamic, DateTime>> timedContacts = _contacts
          .map(
            (c) => MapEntry(
              c,
              Helpers.toPKT(c['latest_message'] ?? c['updated_at']),
            ),
          )
          .toList();
      timedContacts.sort((a, b) => b.value.compareTo(a.value));
  ...
  Future<bool> _sendMediaOptimistic({
    ...
    final lastIncoming = getLastIncomingMessageTime();
    if (lastIncoming != null &&
        Helpers.toUtc(null).difference(lastIncoming).inHours >= 24) {
      debugPrint('🛑 [SEND MEDIA] Blocked by 24h policy locally');
      final tempId =
          'temp_media_policy_${DateTime.now().millisecondsSinceEpoch}';
      final failedMessage = {
        'whatsapp_message_id': tempId,
        'local_id': tempId,
        'message': 'Media',
    ...
    debugPrint('📁 [SEND MEDIA] Exists: ${await file.exists()}');
    debugPrint(
      '📊 [SEND MEDIA] Original Size: ${Helpers.formatFileSize(originalSize)}',
    );
    ...
    if (mediaType == 'video' &&
      ...
      try {
        ...
        debugPrint('⚠️ [SEND MEDIA] Could not detect video duration: $e');
      }
    }
    ...
    try {
      ...
      debugPrint('📤 [SEND MEDIA] Initial uploadPath: $uploadPath');
      if (mediaType == 'video') {
        ...
        try {
          ...
          if (durationSec > 365) {
            // 6 minute limit + 5s buffer
            debugPrint('🛑 [SEND MEDIA] Video too long: ${durationSec}s');
            throw Exception(
              'Video is too long. Maximum duration allowed is 6 minutes.',
            );
          }
        } catch (e) {
          debugPrint(
            '⚠️ [COMPRESS] Could not get media info or limit exceeded: $e',
          );
        ...
        final bool forceCompression =
            Platform.isIOS || !filePath.toLowerCase().endsWith('.mp4');
        debugPrint(
          '📹 [COMPRESS] forceCompression=$forceCompression platformIOS=${Platform.isIOS} ext=${filePath.toLowerCase().split('.').last}',
        );

        // Always compress iOS gallery videos and any non-mp4 input to keep the
        // upload format predictable for the backend/filepond layer.
        if (!forceCompression && originalSize < 1 * 1024 * 1024) {
          debugPrint(
            '📹 [COMPRESS] File is very small (${Helpers.formatFileSize(originalSize)}), skipping compression.',
          );
        } else {
          final MediaInfo? mediaInfo = await VideoCompress.compressVideo(
            filePath,
            quality: VideoQuality
                .LowQuality, // Switched to LowQuality for maximum compatibility
            deleteOrigin: false,
            includeAudio: true,
          );
          ...
          if (mediaInfo?.path != null) {
            ...
            debugPrint(
              '📹 [COMPRESS] New Size: ${Helpers.formatFileSize(compressedSize)} (${((1 - compressedSize / originalSize) * 100).toStringAsFixed(1)}% reduction)',
            );
            debugPrint('📹 [COMPRESS] Output path: $uploadPath');
          } else {
            debugPrint(
              '⚠️ [COMPRESS] Compression returned null path, using original file',
            );
          }
        }
      ...
      debugPrint(
        '📡 [SEND MEDIA] Repository call started with uploadPath: $uploadPath',
      );
      final result = await _repository.sendMedia(
        ...
        onSendProgress: (sent, total) {
          ...
          final index = _messages.indexWhere(
            (m) => m['whatsapp_message_id'] == tempId,
          );
          if (index != -1) {
            ...
            if ((progress - (data['progress'] ?? 0.0)).abs() > 0.02) {
              ...
              _messages = List.from(
                _messages,
              ); // Ensure list reference changes for UI
              notifyListeners();
            }
          }
        },
      ...
      debugPrint(
        '✅ [SEND MEDIA] Repository Success. Result keys: ${result is Map ? result.keys : 'not map'}',
      );
      ...
      if (index != -1 && result is Map) {
        debugPrint(
          '🔄 [SEND MEDIA] Updating optimistic message with server response',
        );
        ...
        if (resultData is Map && serverMsg['whatsapp_message_id'] == null) {
          final dynamic nestedLog =
              resultData['whatsappMessageLogs'] ??
              resultData['whatsapp_message_logs'];
          if (nestedLog is Map && nestedLog.isNotEmpty) {
            final dynamic firstLog = nestedLog.values.first;
            if (firstLog is Map) {
              serverMsg.addAll(Map<String, dynamic>.from(firstLog));
            }
          }
        }
        ...
        if (serverId != null) {
          ...
          if (existingIndex != -1 && existingIndex != index) {
            debugPrint(
              '🚨 [COLLISION] Server ID $serverId ALREADY EXISTS at index $existingIndex. Removing temp media message at $index.',
            );
            _messages.removeAt(index);
          } else {
            // 🛡️ Deep merge __data to preserve optimistic local links if server link is not yet ready
            Map<String, dynamic> mergedData = Helpers.getMessageData(
              _messages[index],
            );
            final Map<String, dynamic> serverData = Helpers.getMessageData(
              serverMsg,
            );
            if (serverData.isNotEmpty) {
              final serverMediaValues = serverData['media_values'];
              if (serverMediaValues is Map) {
                final Map<String, dynamic> localMediaValues = Map.from(
                  mergedData['media_values'] ?? {},
                );

                // Only overwrite link if server provides a valid URL
                final String? serverLink = serverMediaValues['link']
                    ?.toString();
                if (serverLink == null ||
                    serverLink.isEmpty ||
                    !serverLink.startsWith('http')) {
                  serverMediaValues['link'] =
                      localMediaValues['link']; // Keep local path
                }

                localMediaValues.addAll(
                  Map<String, dynamic>.from(serverMediaValues),
                );
            ...
            if ((serverDuration == null || serverDuration <= 0) &&
                localDuration != null &&
                localDuration > 0) {
              updatedMsg['duration'] = localDuration;
              ...
              debugPrint(
                '🛡️ [SEND MEDIA] Preserved local duration: ${localDuration}s for $tempId',
              );
            }
            ...
            _updateContactLatestMessage(contactUid, updatedMsg);
            debugPrint(
              '✅ [SEND MEDIA] Successfully updated temp ID $tempId to server ID $serverId',
            );
          }
        }
        ...
        debugPrint(
          '⚠️ [SEND MEDIA] Could not find optimistic message $tempId to update (Index: $index)',
        );
      }
      ...
      if (e is DioException) {
        if (e.type == DioExceptionType.connectionTimeout ||
          ...
          displayError =
              "Upload timed out. Your video might be too large for your internet connection.";
        } else if (e.response != null) {
          final status = e.response?.statusCode;
          debugPrint(
            '📥 [SEND MEDIA] Server Error Response ($status): ${e.response?.data}',
          );

          if (status == 413) {
            displayError =
                "Video file size exceeds server limit. Try a shorter video.";
          } else if (status == 406) {
      ...
      if ((displayError.contains('2 minute') ||
              displayError.contains('2-minute')) &&
          mediaType == 'video') {
        displayError =
            "Server Error: Video exceeds backend limit (2 mins). Please upload a shorter video.";
      }
  ...
  Future<bool> createLabel({
    ...
    try {
      ...
      final isSuccessful = reaction == 1 || reaction == 14;

      if (!isSuccessful) {
        throw Exception(result['message'] ?? result['incident'] ?? 'Failed to create label');
      }

      // Sync the full list from response if backend provided it
      _syncLabelsFromResponse(result);
      
      // Fallback: If list wasn't in response, manually add if it's in 'data'
      if (result['data'] is Map && (result['data'] as Map).containsKey('uid')) {
         final newLabel = LabelModel.fromJson(result['data']);
         if (!_allAvailableLabels.any((l) => l.uid == newLabel.uid)) {
  ...
  Future<bool> updateLabel({
    ...
    try {
      ...
      final reaction = result['reaction'];
      if (reaction != 1 && reaction != 14) {
        throw Exception(result['message'] ?? 'Failed to update label');
      }
      ...
      final globalIndex = _allAvailableLabels.indexWhere((l) => l.uid == labelUid);
      if (globalIndex != -1) {
        _allAvailableLabels[globalIndex] = updatedLabel;
      }

      // 2. Update Active Chat Labels (Assigned)
      final assignedIndex = _labels.indexWhere((l) => l.uid == labelUid);
      if (assignedIndex != -1) {
        _labels[assignedIndex] = updatedLabel;
      }

      // 3. Update Contact List (Deep update)
      for (var i = 0; i < _contacts.length; i++) {
        final contact = _contacts[i];
        if (contact['labels'] is List) {
          final List<dynamic> contactLabels = contact['labels'];
          for (var j = 0; j < contactLabels.length; j++) {
            final labelData = contactLabels[j];
            final uid = (labelData is Map) 
                ? (labelData['label_uid'] ?? labelData['uid'] ?? labelData['id'] ?? '').toString()
                : (labelData is LabelModel ? labelData.uid : '');
                
            if (uid == labelUid) {
              contactLabels[j] = updatedLabel.toJson();
            }
          }
        }
      }
  ...
  Future<bool> deleteLabel(String labelUid) async {
    ...
    notifyListeners();
    try {
      final result = await _repository.deleteLabel(labelUid, userUid: _activeUserUid);
      
      final reaction = result['reaction'];
      if (reaction != 1 && reaction != 14) {
         throw Exception(result['message'] ?? 'Failed to delete label');
      }
      ...
      // 2. Remove from Active Chat Labels
      _labels.removeWhere((l) => l.uid == labelUid);
      
      // 3. [CASCADING DELETE] Remove this label from all contacts in memory
  ...
  Future<bool> assignLabels({
    ...
    try {
      ...
      final isSuccessful =
          ...
          result['result'] == 'success';
      if (!isSuccessful) {
        throw Exception(result['message'] ?? 'Failed to assign labels');
      }
  ...
  void _syncLabelsFromResponse(dynamic result) {
    if (result is! Map) return;
    
    final clientModels = result['client_models'];
    final data = result['data'];
    
    final dynamic rawLabels = (clientModels is Map ? clientModels['allLabels'] : null) ?? 
                             (data is Map ? data['allLabels'] : null);
                             
    if (rawLabels is List && rawLabels.isNotEmpty) {
      debugPrint('🔄 [SYNC] Synchronizing ${rawLabels.length} labels from server response');
      _allAvailableLabels = rawLabels.map((l) => LabelModel.fromJson(l as Map<String, dynamic>)).toList();
      
      // Update assigned labels if they were modified in the sync
      for (var i = 0; i < _labels.length; i++) {
        final syncedLabel = _allAvailableLabels.cast<LabelModel?>().firstWhere(
          (l) => l?.uid == _labels[i].uid, 
          orElse: () => null
        );
        if (syncedLabel != null) {
          _labels[i] = syncedLabel;
        }
      }
  ...
  List<dynamic> _extractGroupsFromResponse(dynamic result) {
    ...
    final clientModels = result['client_models'];
    final data = result['data'];
    return (clientModels?['contactGroups'] ??
            clientModels?['groups'] ??
            data?['contactGroups'] ??
            data?['groups'] ??
            [])
  ...
  List<dynamic> _extractCountriesFromResponse(dynamic result) {
    ...
    dynamic found =
        clientModels?['countries'] ??
        clientModels?['all_countries'] ??
        data['countries'] ??
        data['all_countries'];
